<?php
if (!function_exists('newses_get_terms')):
function newses_get_terms( $category_id = 0, $taxonomy='category', $default='' ){
    ...
    if ( $category_id > 0 ) {
        ...
        if (isset($terms) && !empty($terms)) {
            foreach ($terms as $term) {
                if( $default != 'first' ){
                    $array['0'] = __('Select Category', 'newses');
                }
 ...
 * @since newses 1.0.0
 */
if (!function_exists('newses_get_terms_link')):
function newses_get_terms_link( $category_id = 0 ){
 ...
 * @since newses 1.0.0
 */
if (!function_exists('newses_get_excerpt')):
    function newses_get_excerpt($length = 25, $newses_content = null, $post_id = 1) {
        $widget_excerpt   = newses_get_option('global_widget_excerpt_setting');
        if($widget_excerpt == 'default-excerpt'){
            return the_excerpt();
        }

        $length          = absint($length);
        $source_content  = preg_replace('`\[[^\]]*\]`', '', $newses_content);
 ...
 * @since newses 1.0.0
 */
if(!function_exists('newses_no_image_url')):
    function newses_no_image_url(){
